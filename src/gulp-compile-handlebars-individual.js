/**
Based on https://www.npmjs.com/package/gulp-compile-handlebars
Goal is similar to http://tompennington.co.uk/generating-multiple-static-html-pages-with-gulp-and-handlebars/

This plugin lets a pipe a bunch of configuration files in - and run handlebars on
a single template passed as an argument on them all.
*/
...
function handlebars(templateToParse, data, opts) {
  ...
  /**
  context = the JSON configuration object.
  templateToParse = the TEMPLATE.
  */
	return through.obj(function (context, enc, cb) {
		...
		if (context.isStream()) {
			this.emit('error', new gutil.PluginError('gulp-compile-handlebars-individual', 'Streaming not supported'));
			return cb();
		}
		...
		try {
			...
			if(options.ignorePartials){
				mockPartials(templateToParse);
			}

			// Enable gulp-data usage, Extend default data with data from file.data
			if(context){
                var contents = JSON.parse(context.contents.toString('utf8'));
				_data = extend(_data, context.contents);
                _data = contents;
			}

      /* somehting missing here- i need to add the context. in the handlebars command.
      */
			//var template = Handlebars.compile(templateToParse, options.compile);
            var template = Handlebars.compile(templateToParse, options.compile);
      //Geneate the output.
			context.contents = new Buffer(template(_data));
		} catch (err) {
			this.emit('error', new gutil.PluginError('gulp-compile-handlebars-individual', err));
		}
